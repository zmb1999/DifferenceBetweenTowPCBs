...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None):
        ...
        opendir = action('&打开文件夹1', lambda :self.openDirDialog(flag = 1),
                         'Ctrl+u', 'open', u'Open Dir1')

        opendir2 = action('&打开文件夹2', lambda :self.openDirDialog(flag = 0),
                         'Ctrl+u', 'open', u'Open Dir1')

        openTemplate = action('&打开样板', self.openTemplate,
                          None, 'open', u'Open Template')

        openExcel = action('&打开Excel', self.openExcel,
                          None, 'open', u'Open Excel')
        ...
        openNextImg = action('&下一张1', lambda :self.openNextImg(flag = 1),
                             'd', 'next', u'Open Next')

        openNextImg2 = action('&下一张2', lambda: self.openNextImg(flag = 0),
                             'd', 'next', u'Open Next')

        openPrevImg = action('&上一张1', lambda:self.openPrevImg(flag = 1),
                             'a', 'prev', u'Open Prev')

        openPrevImg2 = action('&上一张2', lambda: self.openPrevImg(flag = 0),
                             'a', 'prev', u'Open Prev')

        comparePCB = action('比较',lambda :self.compare(flag = 0),None,'comparePCB',u'Compare PCB')
        comparePCBSift = action('通过SIFT算法比较', lambda :self.compare(flag = 1), None, 'comparePCBSift', u'Compare PCB by SIFT')
        ...
        resetAll = action('&重置', self.resetAll, None, 'resetall', u'Reset all')
        ...
        self.actions = struct(save=save, saveTemplate = saveTemplate,resetAll = resetAll,
                              create=create, delete=delete, edit=edit,
                              createMode=createMode, editMode=editMode,comparePCB = comparePCB,comparePCBSift =comparePCBSift,
                              zoom=zoom, zoomIn=zoomIn, zoomOut=zoomOut, zoomOrg=zoomOrg,
                              fitWindow=fitWindow, fitWidth=fitWidth,
                              zoomActions=zoomActions,
                              fileMenuActions=(
                                  opendir, opendir2, openTemplate, openExcel, save, saveTemplate, resetAll, quit),
                              beginner=(), advanced=(),
                              editMenu=(edit, delete,
                                        None, self.drawSquaresOption),
        ...
        self.actions.beginner = (
            opendir, opendir2, openTemplate, openExcel, None, openPrevImg, openPrevImg2, openNextImg, openNextImg2, None, comparePCB, comparePCBSift, None, save, saveTemplate, None,
            create, delete, None,
            zoomIn, zoom, zoomOut, fitWindow, fitWidth)

        self.actions.advanced = (
            opendir, opendir2, openTemplate, openExcel, None, openPrevImg, openPrevImg2, openNextImg, openNextImg2, None, save, saveTemplate, None,
    ...
    def setClean(self):
        if self.mImgList and self.mImgList2:
            self.dirty = False
    ...
    def btnstate(self, item= None):
        ...
        if not item: # If not selected Item, take the first one
            try:
                ...
                item = self.labelList2.item(self.labelList2.count() - 1)
    ...
    def labelSelectionChanged2(self):
        item = self.currentItem2()
        if item and self.canvas2.editing():
            self._noSelectionSlot = True
            self.canvas2.selectShape(self.itemsToShapes[item])
            shape = self.itemsToShapes[item]
            # Add Chris
            self.diffcButton.setChecked(shape.difficult)
    ...
    def newShape(self,flag):
        ...
        if not self.useDefaultLabelCheckbox.isChecked() or not self.defaultLabelTextLine.text():
            ...
            if self.singleClassMode.isChecked() and self.lastLabel:
                ...
                text, _ = self.labelDialog.popUp(text=self.prevLabelText, text2=self.prevLabelText)
                self.lastLabel = text
        else:
        ...
        if text is not None:
            ...
            if flag:
                ...
                shape = self.canvas2.setLastLabel(text, generate_color, generate_color)
            self.addLabel(shape, flag)
    ...
    def openDirDialog(self, _value=False, dirpath=None, flag = None):
        ...
        if flag:
            ...
            self.importDirImages(targetDirPath, flag)
        if flag:
            self.actions.saveTemplate.setEnabled(True)

    def openTemplate(self):
        try:
            with open(".\\data\\TemplateData.json", "r", encoding='utf-8') as jsonFile:
        ...
        if key:
            path = data[key]
            self.importDirImages(path, flag = 1)
        if removeItems:
            for item in removeItems:
                data.pop(item, 'error')
            try:
                with open(".\\data\\TemplateData.json", 'w') as jsonFile:
    ...
    def openExcel(self):
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        filters = "Image & Label files (%s)" % ' '.join(['*%s' % '.xls', '*%s' % '.xlsx'])
        ...
        if filename:
            if isinstance(filename, (tuple, list)):
                filename = filename[0]
        if filename:
            if(self.openRExcel(filename)):
                QMessageBox.information(self, "提示", "导入成功!", QMessageBox.Yes)
            else:
                QMessageBox.warning(self, "警告", "导入失败，请检查Excel文件格式！", QMessageBox.Yes)
    ...
    def saveTemplate(self):
        ...
        if not os.path.exists(".\\data"):
            os.mkdir("data")
        if os.path.exists(".\\data\\TemplateData.json"):
            with open(".\\data\\TemplateData.json", "r", encoding='utf-8') as jsonFile:
                data = json.load(jsonFile)
            flag = True
            print(data)
            ...
            with open(".\\data\\TemplateData.json", 'w') as jsonFile:
                json.dump(data, jsonFile)
            QMessageBox.information(self, "提示", "样本保存成功！", QMessageBox.Yes)
        else:
            with open(".\\data\\TemplateData.json", 'w') as jsonFile:
    ...
    def deleteSelectedShape(self):
        ...
        self.remLabel(self.canvas2.deleteSelected())
        self.setDirty()
        if self.noShapes():
            for action in self.actions.onShapesPresent:
    ...
    #     self.canvas2.endMove(copy=True)
    #     self.addLabel(self.canvas2.selectedShape)
    #     self.setDirty()
    #
    # def moveShape(self):
    ...
    def loadPascalXMLByFilename(self, xmlPath, flag = None):
        if self.filePath is None:
            return
        ...
        self.loadLabels(shapes,flag)
        self.canvas.verified = tVocParseReader.verified
        self.canvas2.verified = tVocParseReader.verified
    ...
    def loadYOLOTXTByFilename(self, txtPath):
        ...
        self.canvas2.verified = tYoloParseReader.verified
    ...
    def compare(self, flag = None):
        ...
        shapesA, shapesB = compare.compare(flag)
        ...
        self.labelList.clear()
        self.labelList2.clear()
        self.loadLabels(sA, 1)
        self.loadLabels(sB, 0)
        self.setDirty()
